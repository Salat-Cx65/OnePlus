...
public class SandParticleShooter {
    ...
    public SandParticleShooter(Vector direction, int color1, int color2) {
    ...
    public void addParticles(SandParticleSystem particleSystem, float currentTime, int count) {
        for (int i = 0; i < count; i++) {
            ...
            float sandGap;
            Point point = new Point(-2.0f, (this.random.nextFloat() * 4.0f) - 2.0f, (this.random.nextFloat() * 2.0f) - 1.0f);
            ...
            Vector thisDirection = this.directionVector.scale(speedAdjustment);
            for (int j = 0; j < 6; j++) {
