...
public class HazeParticleSystem {
    ...
    public void addParticle(Point position, int size, int color, Vector direction, float particleStartTime) {
    ...
    public void bindData(HazeShaderProgram particleProgram) {
        this.vertexArray.setVertexAttribPointer(0, particleProgram.getPositionAttributeLocation(), VECTOR_COMPONENT_COUNT, STRIDE);
        int dataOffset = 0 + 3;
        this.vertexArray.setVertexAttribPointer(dataOffset, particleProgram.getColorAttributeLocation(), VECTOR_COMPONENT_COUNT, STRIDE);
        dataOffset += 3;
        this.vertexArray.setVertexAttribPointer(dataOffset, particleProgram.getDirectionVectorAttributeLocation(), VECTOR_COMPONENT_COUNT, STRIDE);
        dataOffset += 3;
        this.vertexArray.setVertexAttribPointer(dataOffset, particleProgram.getParticleStartTimeAttributeLocation(), SIZE_COMPONENT_COUNT, STRIDE);
        this.vertexArray.setVertexAttribPointer(dataOffset + 1, particleProgram.getSizeAttributeLocation(), SIZE_COMPONENT_COUNT, STRIDE);
    }
