...
public class RainParticles {
    ...
    protected void init(Vector direction) {
        ...
        for (i = 0; i < this.numRect; i++) {
            addRectParticles(i, this.FIRST_HEIGHT_CHANGE_RANGE, AutoScrollHelper.RELATIVE_UNSPECIFIED, AutoScrollHelper.RELATIVE_UNSPECIFIED);
        }
    ...
    public void addRectParticles(int index, float yMagnification, float xRotation, float zRotation) {
        float x = ((((float) this.random.nextInt(10000)) / 10000.0f) * this.X_RANDOM_RANGE) - (this.X_RANDOM_RANGE / 2.0f);
        ...
        float widht = this.RECT_WIDTH_START_OFFSET + (((float) this.random.nextInt(ConnectionResult.DEVELOPER_ERROR)) / 10000.0f);
        float radio = this.RECT_HEIGHT_START_OFFSET + ((((float) this.random.nextInt(10000)) / 10000.0f) * 0.5f);
        float xOffsetDegree = (float) Math.tan(((double) (45.0f - (Math.abs(zRotation) / 2.0f))) * 0.017453292519943295d);
    ...
    protected void addLineParticles(int index, float yMagnification) {
        float x = ((((float) this.random.nextInt(10000)) / 10000.0f) * this.X_RANDOM_RANGE) - (this.X_RANDOM_RANGE / 2.0f);
        float y = ((((((float) this.random.nextInt(10000)) / 10000.0f) * this.Y_RANDOM_RANGE) * yMagnification) + 1.0f) * yMagnification;
        ...
        currentIndex = currentIndex2 + 1;
        this.lineParticles[currentIndex2] = blueColor;
        this.lineWidths[index] = this.WIDTH_CHANGE_RANGE * (this.LINE_WIDTH_START_OFFSET + (((float) this.random.nextInt(10000)) / 10000.0f));
    }

    private void bindData(VertexArray vertexArray, RainShaderProgram particleProgram) {
        vertexArray.setVertexAttribPointer(0, particleProgram.getPositionAttributeLocation(), POSITION_COMPONENT_COUNT, STRIDE);
        int dataOffset = 0 + 3;
        vertexArray.setVertexAttribPointer(dataOffset, particleProgram.getColorAttributeLocation(), POSITION_COMPONENT_COUNT, STRIDE);
        dataOffset += 3;
    }

    public void changePosition(float currentTime, float[] modelProjectionMatrix, int rainDropsTextture, float alpha, float xRotation, float zRotation, float[] modleMatrix) {
        int i;
        for (i = 0; i < this.numLines; i++) {
        ...
        for (i = 0; i < this.numRect; i++) {
            if (this.rectParticles[(i * 24) + 1] < this.Y_OFFSET_LIMIT) {
                ...
                directionx = this.directionVector.x * this.lineSpeeds[i];
                directiony = this.directionVector.y * this.lineSpeeds[i];
