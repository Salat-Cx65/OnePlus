...
public abstract class BaseAnimation {
    ...
    public static float[] orginXYZ() {
        Random random = getRandom();
        float rangeX = ((mCenterZ + (mRangeZ / 2.0f)) - randomRange(random, AutoScrollHelper.RELATIVE_UNSPECIFIED, mRangeZ)) * mRangeX;
    ...
    protected static void nextSeed() {
        INDEX += new Random().nextInt(ConnectionResult.DEVELOPER_ERROR);
    }
}
