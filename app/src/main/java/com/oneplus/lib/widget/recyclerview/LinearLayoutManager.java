...
public class LinearLayoutManager extends LayoutManager implements ViewDropHandler {
    ...
    class AnonymousClass_1 extends LinearSmoothScroller {
    ...
    public void smoothScrollToPosition(RecyclerView recyclerView, State state, int position) {
        LinearSmoothScroller linearSmoothScroller = new AnonymousClass_1(recyclerView.getContext());
    ...
    void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        if (layoutState.mScrapList == null) {
            if (this.mShouldReverseLayout == (layoutState.mLayoutDirection == -1 ? VERTICAL : null)) {
                addView(view);
            } else {
    ...
    public View onFocusSearchFailed(View focused, int focusDirection, Recycler recycler, State state) {
        ...
        View nextFocus;
        ensureLayoutState();
        updateLayoutState(layoutDir, (int) (0.33f * ((float) this.mOrientationHelper.getTotalSpace())), DEBUG, state);
        this.mLayoutState.mScrollingOffset = Integer.MIN_VALUE;
        this.mLayoutState.mRecycle = false;
        ...
        return (nextFocus == referenceChild || !nextFocus.isFocusable()) ? null : nextFocus;
    }
