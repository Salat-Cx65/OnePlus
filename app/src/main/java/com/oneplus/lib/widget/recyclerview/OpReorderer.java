...
class OpReorderer {
    ...
    void swapMoveUpdate(List<UpdateOp> list, int move, UpdateOp moveOp, int update, UpdateOp updateOp) {
        ...
        if (moveOp.itemCount < updateOp.positionStart) {
            ...
            updateOp.itemCount--;
            extraUp1 = this.mCallback.obtainUpdateOp(RainSurfaceView.RAIN_LEVEL_SHOWER, moveOp.positionStart, 1, updateOp.payload);
        }
        if (moveOp.positionStart <= updateOp.positionStart) {
            ...
            extraUp2 = this.mCallback.obtainUpdateOp(RainSurfaceView.RAIN_LEVEL_SHOWER, moveOp.positionStart + 1, remaining, updateOp.payload);
