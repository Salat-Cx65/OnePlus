...
public class SpringConfiguratorView extends FrameLayout {
    ...
    private class SeekbarListener implements OnSeekBarChangeListener {
        ...
        public void onProgressChanged(SeekBar seekBar, int val, boolean b) {
            if (seekBar == SpringConfiguratorView.this.mTensionSeekBar) {
                float scaledTension = ((((float) val) * 200.0f) / 100000.0f) + 0.0f;
                SpringConfiguratorView.this.mSelectedSpringConfig.tension = OrigamiValueConverter.tensionFromOrigamiValue((double) scaledTension);
                SpringConfiguratorView.this.mTensionLabel.setText("T:" + DECIMAL_FORMAT.format((double) scaledTension));
            }
            if (seekBar == SpringConfiguratorView.this.mFrictionSeekBar) {
                float scaledFriction = ((((float) val) * 50.0f) / 100000.0f) + 0.0f;
                SpringConfiguratorView.this.mSelectedSpringConfig.friction = OrigamiValueConverter.frictionFromOrigamiValue((double) scaledFriction);
                SpringConfiguratorView.this.mFrictionLabel.setText("F:" + DECIMAL_FORMAT.format((double) scaledFriction));
            }
        }
    ...
    private class RevealerSpringListener implements SpringListener {
        ...
        public void onSpringUpdate(Spring spring) {
            ...
            SpringConfiguratorView.this.setTranslationY((val * (SpringConfiguratorView.this.mStashPx - minTranslate)) + minTranslate);
        }
    ...
    public SpringConfiguratorView(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mTextColor = Color.argb(MotionEventCompat.ACTION_MASK, 225, 225, 225);
    ...
    private View generateHierarchy(Context context) {
        ...
        container.setBackgroundColor(Color.argb(LocationRequest.PRIORITY_HIGH_ACCURACY, 0, 0, 0));
        ...
        params = Util.createLayoutParams(Util.dpToPx(MAX_FRICTION, resources), -1);
        this.mTensionLabel.setGravity(ConnectionResult.SERVICE_MISSING_PERMISSION);
        ...
        params = Util.createLayoutParams(Util.dpToPx(MAX_FRICTION, resources), -1);
        this.mFrictionLabel.setGravity(ConnectionResult.SERVICE_MISSING_PERMISSION);
        ...
        nub.setBackgroundColor(Color.argb(MotionEventCompat.ACTION_MASK, 0, 164, 209));
        root.addView(nub);
        return root;
    }
    ...
    private void updateSeekBarsForSpringConfig(SpringConfig springConfig) {
        int scaledTension = Math.round(((((float) OrigamiValueConverter.origamiValueFromTension(springConfig.tension)) - 0.0f) * 100000.0f) / 200.0f);
        int scaledFriction = Math.round(((((float) OrigamiValueConverter.origamiValueFromFriction(springConfig.friction)) - 0.0f) * 100000.0f) / 50.0f);
