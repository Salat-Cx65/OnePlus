...
public final class Primitives {
    ...
    public static <T> Class<T> wrap(Class<T> type) {
        Class<T> wrapped = (Class) PRIMITIVE_TO_WRAPPER_TYPE.get(_$Gson$Preconditions.checkNotNull(type));
        return wrapped == null ? type : wrapped;
    }

    public static <T> Class<T> unwrap(Class<T> type) {
        Class<T> unwrapped = (Class) WRAPPER_TO_PRIMITIVE_TYPE.get(_$Gson$Preconditions.checkNotNull(type));
