...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void parseInterpolatorFromTypeArray(TypedArray a, XmlPullParser parser) {
        if (TypedArrayUtils.hasAttribute(parser, "pathData")) {
            String pathData = TypedArrayUtils.getNamedString(a, parser, "pathData", RainSurfaceView.RAIN_LEVEL_RAINSTORM);
            ...
            float x1 = TypedArrayUtils.getNamedFloat(a, parser, "controlX1", 0, AutoScrollHelper.RELATIVE_UNSPECIFIED);
            float y1 = TypedArrayUtils.getNamedFloat(a, parser, "controlY1", 1, AutoScrollHelper.RELATIVE_UNSPECIFIED);
            ...
            if (hasX2 != TypedArrayUtils.hasAttribute(parser, "controlY2")) {
                ...
                initCubic(x1, y1, TypedArrayUtils.getNamedFloat(a, parser, "controlX2", RainSurfaceView.RAIN_LEVEL_SHOWER, AutoScrollHelper.RELATIVE_UNSPECIFIED), TypedArrayUtils.getNamedFloat(a, parser, "controlY2", RainSurfaceView.RAIN_LEVEL_DOWNPOUR, AutoScrollHelper.RELATIVE_UNSPECIFIED));
            } else {
                initQuad(x1, y1);
            }
        } else {
    ...
    private void initCubic(float x1, float y1, float x2, float y2) {
        Path path = new Path();
        path.moveTo(AutoScrollHelper.RELATIVE_UNSPECIFIED, AutoScrollHelper.RELATIVE_UNSPECIFIED);
        path.cubicTo(x1, y1, x2, y2, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 1.0f);
        initPath(path);
    }
    ...
    private void initPath(Path path) {
        ...
        float prevX = AutoScrollHelper.RELATIVE_UNSPECIFIED;
        i = 0;
