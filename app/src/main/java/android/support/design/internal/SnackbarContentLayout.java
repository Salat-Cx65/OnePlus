...
public class SnackbarContentLayout extends LinearLayout implements ContentViewCallback {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mMaxWidth > 0 && getMeasuredWidth() > this.mMaxWidth) {
            widthMeasureSpec = MeasureSpec.makeMeasureSpec(this.mMaxWidth, CitySearchProvider.GET_SEARCH_RESULT_SUCC);
            super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        }
    ...
    public void animateContentIn(int delay, int duration) {
        this.mMessageView.setAlpha(AutoScrollHelper.RELATIVE_UNSPECIFIED);
        this.mMessageView.animate().alpha(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT).setDuration((long) duration).setStartDelay((long) delay).start();
        if (this.mActionView.getVisibility() == 0) {
            this.mActionView.setAlpha(AutoScrollHelper.RELATIVE_UNSPECIFIED);
            this.mActionView.animate().alpha(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT).setDuration((long) duration).setStartDelay((long) delay).start();
        }
    }

    public void animateContentOut(int delay, int duration) {
        this.mMessageView.setAlpha(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        this.mMessageView.animate().alpha(AutoScrollHelper.RELATIVE_UNSPECIFIED).setDuration((long) duration).setStartDelay((long) delay).start();
        if (this.mActionView.getVisibility() == 0) {
            this.mActionView.setAlpha(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
            this.mActionView.animate().alpha(AutoScrollHelper.RELATIVE_UNSPECIFIED).setDuration((long) duration).setStartDelay((long) delay).start();
        }
    }
}
