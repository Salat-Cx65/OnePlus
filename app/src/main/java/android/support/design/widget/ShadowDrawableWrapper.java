...
class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    private void drawShadow(Canvas canvas) {
        ...
        if (drawHorizontalEdges) {
            ...
            canvas.drawRect(AutoScrollHelper.RELATIVE_UNSPECIFIED, edgeShadowTop, this.mContentBounds.width() - (2.0f * shadowOffset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        ...
        if (drawHorizontalEdges) {
            canvas.scale(1.0f / shadowScaleHorizontal, SHADOW_BOTTOM_SCALE);
            canvas.drawRect(AutoScrollHelper.RELATIVE_UNSPECIFIED, edgeShadowTop, this.mContentBounds.width() - (2.0f * shadowOffset), this.mShadowSize + (-this.mCornerRadius), this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
        ...
        if (drawVerticalEdges) {
            ...
            canvas.drawRect(AutoScrollHelper.RELATIVE_UNSPECIFIED, edgeShadowTop, this.mContentBounds.height() - (2.0f * shadowOffset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
