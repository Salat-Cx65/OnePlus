...
public class CoordinatorLayout extends ViewGroup implements NestedScrollingParent2 {
    ...
    public static abstract class Behavior<V extends View> {
        ...
        public boolean onStartNestedScroll(@NonNull CoordinatorLayout coordinatorLayout, @NonNull V child, @NonNull View directTargetChild, @NonNull View target, int axes, int type) {
            return type == 0 ? onStartNestedScroll(coordinatorLayout, child, directTargetChild, target, axes) : false;
        }
    ...
    private class HierarchyChangeListener implements OnHierarchyChangeListener {
        ...
        public void onChildViewRemoved(View parent, View child) {
            CoordinatorLayout.this.onChildViewsChanged(EVENT_VIEW_REMOVED);
            if (CoordinatorLayout.this.mOnHierarchyChangeListener != null) {
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        LayoutParams(Context context, AttributeSet attrs) {
            ...
            this.gravity = a.getInteger(R.styleable.CoordinatorLayout_Layout_android_layout_gravity, TYPE_ON_INTERCEPT);
            this.mAnchorId = a.getResourceId(R.styleable.CoordinatorLayout_Layout_layout_anchor, -1);
            this.anchorGravity = a.getInteger(R.styleable.CoordinatorLayout_Layout_layout_anchorGravity, TYPE_ON_INTERCEPT);
            this.keyline = a.getInteger(R.styleable.CoordinatorLayout_Layout_layout_keyline, -1);
            this.insetEdge = a.getInt(R.styleable.CoordinatorLayout_Layout_layout_insetEdge, TYPE_ON_INTERCEPT);
            this.dodgeInsetEdges = a.getInt(R.styleable.CoordinatorLayout_Layout_layout_dodgeInsetEdges, TYPE_ON_INTERCEPT);
        ...
        boolean isBlockingInteractionBelow(CoordinatorLayout parent, View child) {
            ...
            boolean blocksInteractionBelow = (this.mBehavior != null ? this.mBehavior.blocksInteractionBelow(parent, child) : TYPE_ON_INTERCEPT) | this.mDidBlockInteraction;
        ...
        private boolean shouldDodge(View other, int layoutDirection) {
            int absInset = GravityCompat.getAbsoluteGravity(((android.support.design.widget.CoordinatorLayout.LayoutParams) other.getLayoutParams()).insetEdge, layoutDirection);
    ...
    class OnPreDrawListener implements android.view.ViewTreeObserver.OnPreDrawListener {
        ...
        public boolean onPreDraw() {
            CoordinatorLayout.this.onChildViewsChanged(TYPE_ON_INTERCEPT);
            return true;
        }
    }
    ...
    static class ViewElevationComparator implements Comparator<View> {
        ...
        public int compare(View lhs, View rhs) {
            ...
            return lz < rz ? TYPE_ON_TOUCH : TYPE_ON_INTERCEPT;
        }
    }
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public SavedState(Parcel source, ClassLoader loader) {
            ...
            for (int i = TYPE_ON_INTERCEPT; i < size; i++) {
        ...
        public void writeToParcel(Parcel dest, int flags) {
            ...
            int size = this.behaviorStates != null ? this.behaviorStates.size() : TYPE_ON_INTERCEPT;
            dest.writeInt(size);
            int[] ids = new int[size];
    ...
    public CoordinatorLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (keylineArrayRes != 0) {
            ...
            for (int i = TYPE_ON_INTERCEPT; i < count; i++) {
    ...
    private void resetTouchBehaviors() {
        if (this.mBehaviorTouchView != null) {
            ...
            if (b != null) {
                ...
                MotionEvent cancelEvent = MotionEvent.obtain(now, now, RainSurfaceView.RAIN_LEVEL_DOWNPOUR, AutoScrollHelper.RELATIVE_UNSPECIFIED, 0.0f, TYPE_ON_INTERCEPT);
        ...
        for (int i = TYPE_ON_INTERCEPT; i < childCount; i++) {
    ...
    private boolean performIntercept(MotionEvent ev, int type) {
        ...
        for (int i = TYPE_ON_INTERCEPT; i < childCount; i++) {
            ...
            if ((!intercepted && !newBlock) || action == 0) {
                ...
                if (cancelEvent == null) {
                    long now = SystemClock.uptimeMillis();
                    cancelEvent = MotionEvent.obtain(now, now, RainSurfaceView.RAIN_LEVEL_DOWNPOUR, AutoScrollHelper.RELATIVE_UNSPECIFIED, AutoScrollHelper.RELATIVE_UNSPECIFIED, TYPE_ON_INTERCEPT);
                }
                switch (type) {
                    case TYPE_ON_INTERCEPT:
    ...
    static Behavior parseBehavior(Context context, AttributeSet attrs, String name) {
        ...
        } else if (name.indexOf(net.oneplus.weather.R.styleable.AppCompatTheme_checkboxStyle) >= 0) {
            fullName = name;
        } else {
    ...
    private void prepareChildren() {
        ...
        for (int i = TYPE_ON_INTERCEPT; i < count; i++) {
            ...
            for (int j = TYPE_ON_INTERCEPT; j < count; j++) {
                if (j != i) {
    ...
    private WindowInsetsCompat dispatchApplyWindowInsetsToBehaviors(WindowInsetsCompat insets) {
        ...
        int z = getChildCount();
        for (int i = TYPE_ON_INTERCEPT; i < z; i++) {
    ...
    private static int resolveAnchoredChildGravity(int gravity) {
        return gravity == 0 ? ConnectionResult.SIGN_IN_FAILED : gravity;
    }
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (lp.mBehavior != null) {
            ...
            if (scrimAlpha > 0.0f) {
                ...
                this.mScrimPaint.setAlpha(MathUtils.clamp(Math.round(255.0f * scrimAlpha), (int) TYPE_ON_INTERCEPT, (int) MotionEventCompat.ACTION_MASK));
    ...
    public void dispatchDependentViewsChanged(View view) {
        ...
        if (dependents != null && !dependents.isEmpty()) {
            for (int i = TYPE_ON_INTERCEPT; i < dependents.size(); i++) {
    ...
    public boolean onStartNestedScroll(View child, View target, int nestedScrollAxes) {
        return onStartNestedScroll(child, target, nestedScrollAxes, TYPE_ON_INTERCEPT);
    }
    ...
    public void onNestedScrollAccepted(View child, View target, int nestedScrollAxes) {
        onNestedScrollAccepted(child, target, nestedScrollAxes, TYPE_ON_INTERCEPT);
    }
    ...
    public void onStopNestedScroll(View target) {
        onStopNestedScroll(target, TYPE_ON_INTERCEPT);
    }
    ...
    public void onNestedScroll(View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        onNestedScroll(target, dxConsumed, dyConsumed, dxUnconsumed, dyUnconsumed, TYPE_ON_INTERCEPT);
    }
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        onNestedPreScroll(target, dx, dy, consumed, TYPE_ON_INTERCEPT);
    }
    ...
    public boolean onNestedFling(View target, float velocityX, float velocityY, boolean consumed) {
        ...
        for (int i = TYPE_ON_INTERCEPT; i < childCount; i++) {
            ...
            if (view.getVisibility() != 8) {
                LayoutParams lp = (LayoutParams) view.getLayoutParams();
                if (lp.isNestedScrollAccepted(TYPE_ON_INTERCEPT)) {
                    Behavior viewBehavior = lp.getBehavior();
                    if (viewBehavior != null) {
    ...
    public boolean requestChildRectangleOnScreen(View child, Rect rectangle, boolean immediate) {
        Behavior behavior = ((LayoutParams) child.getLayoutParams()).getBehavior();
        return (behavior == null || !behavior.onRequestChildRectangleOnScreen(this, child, rectangle, immediate)) ? super.requestChildRectangleOnScreen(child, rectangle, immediate) : true;
    }
