...
public final class Snackbar extends BaseTransientBottomBar<Snackbar> {
    ...
    public static final class SnackbarLayout extends SnackbarBaseLayout {
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            for (int i = LENGTH_LONG; i < childCount; i++) {
                View child = getChildAt(i);
                if (child.getLayoutParams().width == -1) {
                    child.measure(MeasureSpec.makeMeasureSpec(availableWidth, CitySearchProvider.GET_SEARCH_RESULT_SUCC), MeasureSpec.makeMeasureSpec(child.getMeasuredHeight(), CitySearchProvider.GET_SEARCH_RESULT_SUCC));
                }
            }
        }
    }
    ...
    public Snackbar setAction(CharSequence text, OnClickListener listener) {
        ...
        if (TextUtils.isEmpty(text) || listener == null) {
            tv.setVisibility(DetectedActivity.RUNNING);
            tv.setOnClickListener(null);
        } else {
            tv.setVisibility(LENGTH_LONG);
            tv.setText(text);
            tv.setOnClickListener(new AnonymousClass_1(listener));
        }
        return this;
    }
