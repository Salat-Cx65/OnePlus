...
public class DiffUtil {
    ...
    public static class DiffResult {
        ...
        class AnonymousClass_1 implements ListUpdateCallback {
            final /* synthetic */ Adapter val$adapter;

            AnonymousClass_1(Adapter adapter) {
        ...
        public void dispatchUpdatesTo(Adapter adapter) {
            dispatchUpdatesTo(new AnonymousClass_1(adapter));
        }
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.x + i, FLAG_NOT_CHANGED, this.mCallback.getChangePayload(snake.x + i, snake.y + i));
                    }
                }
                posOld = snake.x;
                posNew = snake.y;
