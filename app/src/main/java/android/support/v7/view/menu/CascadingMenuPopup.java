...
final class CascadingMenuPopup extends MenuPopup implements MenuPresenter, OnKeyListener, OnDismissListener {
    ...
    public CascadingMenuPopup(@NonNull Context context, @NonNull View anchor, @AttrRes int popupStyleAttr, @StyleRes int popupStyleRes, boolean overflowOnly) {
        ...
        this.mGlobalLayoutListener = new OnGlobalLayoutListener() {
            public void onGlobalLayout() {
                if (CascadingMenuPopup.this.isShowing() && CascadingMenuPopup.this.mShowingMenus.size() > 0 && !((CascadingMenuInfo) CascadingMenuPopup.this.mShowingMenus.get(HORIZ_POSITION_LEFT)).window.isModal()) {
        ...
        this.mMenuItemHoverListener = new MenuItemHoverListener() {
            ...
            class AnonymousClass_1 implements Runnable {
                ...
                AnonymousClass_1(CascadingMenuInfo cascadingMenuInfo, MenuItem menuItem, MenuBuilder menuBuilder) {
                ...
                public void run() {
                    ...
                    if (this.val$item.isEnabled() && this.val$item.hasSubMenu()) {
                        this.val$menu.performItemAction(this.val$item, RainSurfaceView.RAIN_LEVEL_RAINSTORM);
                    }
                }
            }
            ...
            public void onItemHoverEnter(@NonNull MenuBuilder menu, @NonNull MenuItem item) {
                ...
                for (int i = HORIZ_POSITION_LEFT; i < count; i++) {
                ...
                if (menuIndex != -1) {
                    ...
                    CascadingMenuPopup.this.mSubMenuHoverHandler.postAtTime(new AnonymousClass_1(nextInfo, item, menu), menu, SystemClock.uptimeMillis() + 200);
                }
            }
        };
    ...
    private MenuPopupWindow createPopupWindow() {
        ...
        popupWindow.setInputMethodMode(RainSurfaceView.RAIN_LEVEL_SHOWER);
        return popupWindow;
    }
    ...
    private int getInitialMenuPosition() {
        return ViewCompat.getLayoutDirection(this.mAnchorView) == 1 ? HORIZ_POSITION_LEFT : HORIZ_POSITION_RIGHT;
    }
    ...
    private void showMenu(@NonNull MenuBuilder menu) {
        ...
        if (parentView != null) {
            ...
            if (VERSION.SDK_INT >= 26) {
                ...
                parentOffsetLeft = HORIZ_POSITION_LEFT;
                parentOffsetTop = HORIZ_POSITION_LEFT;
            } else {
    ...
    private View findParentViewForSubmenu(@NonNull CascadingMenuInfo parentInfo, @NonNull MenuBuilder submenu) {
        ...
        return (ownerViewPosition < 0 || ownerViewPosition >= listView.getChildCount()) ? null : listView.getChildAt(ownerViewPosition);
    }
