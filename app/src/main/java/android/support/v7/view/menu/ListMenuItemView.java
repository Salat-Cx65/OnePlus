...
public class ListMenuItemView extends LinearLayout implements ItemView {
    ...
    public void setTitle(CharSequence title) {
        if (title != null) {
            ...
            this.mTitleView.setVisibility(DetectedActivity.RUNNING);
        }
    }
        ...
        if (checkable || this.mRadioButton != null || this.mCheckBox != null) {
            ...
            if (checkable) {
                ...
                if (otherCompoundButton != null && otherCompoundButton.getVisibility() != 8) {
                    otherCompoundButton.setVisibility(DetectedActivity.RUNNING);
                    return;
                }
                return;
            }
            if (this.mCheckBox != null) {
                this.mCheckBox.setVisibility(DetectedActivity.RUNNING);
            }
            if (this.mRadioButton != null) {
                this.mRadioButton.setVisibility(DetectedActivity.RUNNING);
            }
        }
    }
    ...
    private void setSubMenuArrowVisible(boolean hasSubmenu) {
        if (this.mSubMenuArrowView != null) {
            this.mSubMenuArrowView.setVisibility(hasSubmenu ? 0 : DetectedActivity.RUNNING);
        }
    }

    public void setShortcut(boolean showShortcut, char shortcutKey) {
        int newVisibility = (showShortcut && this.mItemData.shouldShowShortcut()) ? 0 : DetectedActivity.RUNNING;
        if (newVisibility == 0) {
