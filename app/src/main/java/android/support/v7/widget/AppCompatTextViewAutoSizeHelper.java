...
class AppCompatTextViewAutoSizeHelper {
        ...
        if (!supportsAutoSizeText()) {
            ...
            if (!this.mHasPresetAutoSizeValues) {
                ...
                if (autoSizeMinTextSizeInPx == -1.0f) {
                    autoSizeMinTextSizeInPx = TypedValue.applyDimension(RainSurfaceView.RAIN_LEVEL_SHOWER, 12.0f, displayMetrics);
                }
                if (autoSizeMaxTextSizeInPx == -1.0f) {
                    autoSizeMaxTextSizeInPx = TypedValue.applyDimension(RainSurfaceView.RAIN_LEVEL_SHOWER, 112.0f, displayMetrics);
                }
                if (autoSizeStepGranularityInPx == -1.0f) {
                    autoSizeStepGranularityInPx = DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
                }
    ...
    void setAutoSizeTextTypeWithDefaults(int autoSizeTextType) {
        if (supportsAutoSizeText()) {
            switch (autoSizeTextType) {
                ...
                case DEFAULT_AUTO_SIZE_GRANULARITY_IN_PX:
                    DisplayMetrics displayMetrics = this.mContext.getResources().getDisplayMetrics();
                    validateAndSetAutoSizeTextTypeUniformConfiguration(TypedValue.applyDimension(RainSurfaceView.RAIN_LEVEL_SHOWER, 12.0f, displayMetrics), TypedValue.applyDimension(RainSurfaceView.RAIN_LEVEL_SHOWER, 112.0f, displayMetrics), DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    ...
    private boolean setupAutoSizeText() {
        if (supportsAutoSizeText() && this.mAutoSizeTextType == 1) {
            if (!this.mHasPresetAutoSizeValues || this.mAutoSizeTextSizesInPx.length == 0) {
                int autoSizeValuesLength = DEFAULT_AUTO_SIZE_GRANULARITY_IN_PX;
    ...
    void autoSizeText() {
        ...
        if (this.mNeedsAutoSizeText && (this.mTextView.getMeasuredHeight() > 0 && this.mTextView.getMeasuredWidth() > 0)) {
            int availableWidth = ((Boolean) invokeAndReturnWithDefault(this.mTextView, "getHorizontallyScrolling", Boolean.valueOf(false))).booleanValue() ? VERY_WIDE : (this.mTextView.getMeasuredWidth() - this.mTextView.getTotalPaddingLeft()) - this.mTextView.getTotalPaddingRight();
    ...
    private StaticLayout createStaticLayoutForMeasuring(CharSequence text, Alignment alignment, int availableWidth, int maxLines) {
        ...
        if (maxLines == -1) {
            maxLines = Preference.DEFAULT_ORDER;
        }
    ...
    private StaticLayout createStaticLayoutForMeasuringPre23(CharSequence text, Alignment alignment, int availableWidth) {
        ...
        if (VERSION.SDK_INT >= 16) {
            ...
            lineSpacingMultiplier = ((Float) invokeAndReturnWithDefault(this.mTextView, "getLineSpacingMultiplier", Float.valueOf(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT))).floatValue();
            lineSpacingAdd = ((Float) invokeAndReturnWithDefault(this.mTextView, "getLineSpacingExtra", Float.valueOf(AutoScrollHelper.RELATIVE_UNSPECIFIED))).floatValue();
    ...
    private <T> T invokeAndReturnWithDefault(@NonNull Object object, @NonNull String methodName, @NonNull T defaultValue) {
        try {
            ...
            return (result == null && false) ? defaultValue : result;
        } catch (Exception ex) {
            try {
                Log.w(TAG, "Failed to invoke TextView#" + methodName + "() method", ex);
                return (null != null || 1 == null) ? null : defaultValue;
            } catch (Throwable th) {
                if (null == null && 1 != null) {
                    result = defaultValue;
                }
