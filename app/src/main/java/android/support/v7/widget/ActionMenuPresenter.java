...
class ActionMenuPresenter extends BaseMenuPresenter implements SubUiVisibilityListener {
    ...
    private class OverflowMenuButton extends AppCompatImageView implements ActionMenuChildView {
        ...
        class AnonymousClass_1 extends ForwardingListener {
            ...
            AnonymousClass_1(View src, ActionMenuPresenter actionMenuPresenter) {
        ...
        public OverflowMenuButton(Context context) {
            ...
            setOnTouchListener(new AnonymousClass_1(this, ActionMenuPresenter.this));
        }
    ...
    public View getItemView(MenuItemImpl item, View convertView, ViewGroup parent) {
        ...
        actionView.setVisibility(item.isActionViewExpanded() ? DetectedActivity.RUNNING : 0);
    ...
    public boolean flagActionItems() {
        ...
        if (this.mReserveOverflow) {
            if (hasOverflow || requiredItems + requestedItems > maxActions) {
                maxActions--;
            }
        }
        ...
        if (this.mStrictWidthLimit) {
            i2 = widthLimit / this.mMinCellSize;
            cellSize = this.mMinCellSize + ((widthLimit % this.mMinCellSize) / i2);
        }
        for (i = 0; i < itemsSize; i++) {
            ...
            if (item.requiresActionButton()) {
                ...
                if (this.mStrictWidthLimit) {
                    i2 -= ActionMenuView.measureChildForCells(v, cellSize, i2, querySpec, 0);
                } else {
                ...
                boolean isAction = (maxActions > 0 || inGroup) && widthLimit > 0 && (!this.mStrictWidthLimit || i2 > 0);
                if (isAction) {
                    v = getItemView(item, this.mScrapActionButtonView, parent);
                    if (this.mScrapActionButtonView == null) {
                        this.mScrapActionButtonView = v;
                    ...
                    if (this.mStrictWidthLimit) {
                        int cells = ActionMenuView.measureChildForCells(v, cellSize, i2, querySpec, 0);
                        i2 -= cells;
                        if (cells == 0) {
                            isAction = false;
                        }
