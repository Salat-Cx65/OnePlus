...
public class StaggeredGridLayoutManager extends LayoutManager implements ScrollVectorProvider {
    ...
    class AnchorInfo {
        ...
        void saveSpanReferenceLines(Span[] spans) {
            ...
            for (int i = HORIZONTAL; i < spanCount; i++) {
                this.mSpanReferenceLines[i] = spans[i].getStartLine(INVALID_OFFSET);
            }
        }
    ...
    public static class SavedState implements Parcelable {
        ...
        static {
            CREATOR = new Creator<android.support.v7.widget.StaggeredGridLayoutManager.SavedState>() {
                public android.support.v7.widget.StaggeredGridLayoutManager.SavedState createFromParcel(Parcel in) {
    ...
    class Span {
        ...
        public int findFirstPartiallyVisibleItemPosition() {
            return StaggeredGridLayoutManager.this.mReverseLayout ? findOnePartiallyVisibleChild(this.mViews.size() - 1, -1, true) : findOnePartiallyVisibleChild(HORIZONTAL, this.mViews.size(), true);
        }
        ...
        public int findLastPartiallyVisibleItemPosition() {
            return StaggeredGridLayoutManager.this.mReverseLayout ? findOnePartiallyVisibleChild(HORIZONTAL, this.mViews.size(), true) : findOnePartiallyVisibleChild(this.mViews.size() - 1, -1, true);
        }
        ...
        int findOnePartiallyOrCompletelyVisibleChild(int fromIndex, int toIndex, boolean completelyVisible, boolean acceptCompletelyVisible, boolean acceptEndPointInclusion) {
            ...
            for (int i = fromIndex; i != toIndex; i += next) {
                ...
                boolean childStartInclusion = acceptEndPointInclusion ? childStart <= end ? true : DEBUG : childStart < end ? true : DEBUG;
                boolean childEndInclusion = acceptEndPointInclusion ? childEnd >= start ? true : DEBUG : childEnd > start ? true : DEBUG;
        ...
        int findOneVisibleChild(int fromIndex, int toIndex, boolean completelyVisible) {
            return findOnePartiallyOrCompletelyVisibleChild(fromIndex, toIndex, completelyVisible, true, DEBUG);
        }

        int findOnePartiallyVisibleChild(int fromIndex, int toIndex, boolean acceptEndPointInclusion) {
            return findOnePartiallyOrCompletelyVisibleChild(fromIndex, toIndex, DEBUG, false, acceptEndPointInclusion);
        }
            ...
            if (layoutDir != -1) {
                ...
                while (i >= 0) {
                    ...
                    if (!StaggeredGridLayoutManager.this.mReverseLayout || StaggeredGridLayoutManager.this.getPosition(view) < referenceChildPosition) {
                        if ((!StaggeredGridLayoutManager.this.mReverseLayout && StaggeredGridLayoutManager.this.getPosition(view) <= referenceChildPosition) || !view.hasFocusable()) {
                            break;
                        }
                        candidate = view;
                        i--;
    ...
    private boolean checkSpanForGap(Span span) {
        if (this.mShouldReverseLayout) {
            ...
            return !span.getLayoutParams((View) span.mViews.get(HORIZONTAL)).mFullSpan;
        }
        return false;
    }
    ...
    private void repositionToWrapContentIfNecessary() {
        if (this.mSecondaryOrientation.getMode() != 1073741824) {
            int i;
            View child;
            float maxSize = AutoScrollHelper.RELATIVE_UNSPECIFIED;
            int childCount = getChildCount();
            for (i = HORIZONTAL; i < childCount; i++) {
                child = getChildAt(i);
    ...
    private void measureChildWithDecorationsAndMargin(View child, LayoutParams lp, boolean alreadyMeasured) {
        if (lp.mFullSpan) {
            if (this.mOrientation == 1) {
                measureChildWithDecorationsAndMargin(child, this.mFullSizeSpec, getChildMeasureSpec(getHeight(), getHeightMode(), HORIZONTAL, lp.height, true), alreadyMeasured);
            } else {
                measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(getWidth(), getWidthMode(), HORIZONTAL, lp.width, true), this.mFullSizeSpec, alreadyMeasured);
            }
        } else if (this.mOrientation == 1) {
            measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(this.mSizePerSpan, getWidthMode(), HORIZONTAL, lp.width, DEBUG), getChildMeasureSpec(getHeight(), getHeightMode(), HORIZONTAL, lp.height, true), alreadyMeasured);
        } else {
            measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(getWidth(), getWidthMode(), HORIZONTAL, lp.width, true), getChildMeasureSpec(this.mSizePerSpan, getHeightMode(), HORIZONTAL, lp.height, DEBUG), alreadyMeasured);
        }
    }
    ...
    private int updateSpecWithExtra(int spec, int startInset, int endInset) {
        ...
        return (mode == Integer.MIN_VALUE || mode == 1073741824) ? MeasureSpec.makeMeasureSpec(Math.max(HORIZONTAL, (MeasureSpec.getSize(spec) - startInset) - endInset), mode) : spec;
    }
    ...
    public void onItemsMoved(RecyclerView recyclerView, int from, int to, int itemCount) {
        handleUpdate(from, to, DetectedActivity.RUNNING);
    }

    public void onItemsUpdated(RecyclerView recyclerView, int positionStart, int itemCount, Object payload) {
        handleUpdate(positionStart, itemCount, RainSurfaceView.RAIN_LEVEL_RAINSTORM);
    }
    ...
    private int fill(Recycler recycler, LayoutState layoutState, State state) {
        ...
        if (this.mLayoutState.mInfinite) {
            targetLine = layoutState.mLayoutDirection == 1 ? Preference.DEFAULT_ORDER : INVALID_OFFSET;
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, State state, LayoutPrefetchRegistry layoutPrefetchRegistry) {
        ...
        if (getChildCount() != 0 && delta != 0) {
            ...
            int itemPrefetchCount = HORIZONTAL;
            for (i = HORIZONTAL; i < this.mSpanCount; i++) {
                int distance;
            ...
            Arrays.sort(this.mPrefetchDistances, HORIZONTAL, itemPrefetchCount);
            for (i = HORIZONTAL; i < itemPrefetchCount && this.mLayoutState.hasMore(state); i++) {
    ...
    public View onFocusSearchFailed(View focused, int direction, Recycler recycler, State state) {
        ...
        boolean shouldSearchFromStart = obj == (layoutDir == -1 ? VERTICAL : null) ? true : DEBUG;
        if (!prevFocusFullSpan) {
