...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public int getRowCountForAccessibility(Recycler recycler, State state) {
        ...
        return state.getItemCount() < 1 ? 0 : getSpanGroupIndex(recycler, state, state.getItemCount() - 1) + 1;
    }
    ...
    void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        int i;
        int maxSize = 0;
        float maxSizeInOther = AutoScrollHelper.RELATIVE_UNSPECIFIED;
        ...
        if (flexibleInOtherDir) {
            ...
            for (i = 0; i < count; i++) {
                view = this.mSet[i];
                measureChild(view, CitySearchProvider.GET_SEARCH_RESULT_SUCC, true);
        ...
        for (i = 0; i < count; i++) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view) != maxSize) {
                ...
                if (this.mOrientation == 1) {
                    wSpec = getChildMeasureSpec(totalSpaceInOther, CitySearchProvider.GET_SEARCH_RESULT_SUCC, horizontalInsets, lp.width, DEBUG);
                    hSpec = MeasureSpec.makeMeasureSpec(maxSize - verticalInsets, CitySearchProvider.GET_SEARCH_RESULT_SUCC);
                } else {
                    wSpec = MeasureSpec.makeMeasureSpec(maxSize - horizontalInsets, CitySearchProvider.GET_SEARCH_RESULT_SUCC);
                    hSpec = getChildMeasureSpec(totalSpaceInOther, CitySearchProvider.GET_SEARCH_RESULT_SUCC, verticalInsets, lp.height, DEBUG);
                }
        ...
        for (i = 0; i < count; i++) {
            ...
            layoutDecoratedWithMargins(view, left, i2, right, i3);
    ...
    public View onFocusSearchFailed(View focused, int focusDirection, Recycler recycler, State state) {
        ...
        int inc;
        int limit;
        if ((convertFocusDirectionToLayoutDirection(focusDirection) == 1 ? 1 : null) != this.mShouldReverseLayout ? true : DEBUG) {
        ...
        boolean preferLastSpan = (this.mOrientation == 1 && isLayoutRTL()) ? true : DEBUG;
        View focusableWeakCandidate = null;
        int focusableWeakCandidateSpanIndex = DEFAULT_SPAN_COUNT;
        int focusableWeakCandidateOverlap = 0;
        View unfocusableWeakCandidate = null;
        int unfocusableWeakCandidateSpanIndex = DEFAULT_SPAN_COUNT;
                ...
                if (!(candidate.hasFocusable() && focusableWeakCandidate == null) && (candidate.hasFocusable() || unfocusableWeakCandidate != null)) {
                    ...
                    if (candidate.hasFocusable()) {
                        if (overlap > focusableWeakCandidateOverlap) {
                            ...
                            if (preferLastSpan == (candidateStart > focusableWeakCandidateSpanIndex ? 1 : null)) {
                                assignAsWeek = true;
                            }
                        }
                        ...
                        if (overlap > unfocusableWeakCandidateOverlap) {
                            ...
                            if (preferLastSpan == (candidateStart > unfocusableWeakCandidateSpanIndex ? 1 : null)) {
                                assignAsWeek = true;
                            }
                        }
                    }
