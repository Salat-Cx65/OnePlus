...
public class LinearSnapHelper extends SnapHelper {
    ...
    public int findTargetSnapPosition(LayoutManager layoutManager, int velocityX, int velocityY) {
        ...
        return targetPos >= itemCount ? itemCount - 1 : targetPos;
    }
    ...
    private float computeDistancePerChild(LayoutManager layoutManager, OrientationHelper helper) {
        ...
        int maxPos = CitySearchProvider.GET_SEARCH_RESULT_FAIL;
        int childCount = layoutManager.getChildCount();
        if (childCount == 0) {
        ...
        return distance == 0 ? INVALID_DISTANCE : (1.0f * ((float) distance)) / ((float) ((maxPos - minPos) + 1));
    }
