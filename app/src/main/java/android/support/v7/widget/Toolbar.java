...
public class Toolbar extends ViewGroup {
    ...
    public static class SavedState extends AbsSavedState {
        ...
        static {
            CREATOR = new ClassLoaderCreator<android.support.v7.widget.Toolbar.SavedState>() {
                public android.support.v7.widget.Toolbar.SavedState createFromParcel(Parcel in, ClassLoader loader) {
                    return new android.support.v7.widget.Toolbar.SavedState(in, loader);
                }

                public android.support.v7.widget.Toolbar.SavedState createFromParcel(Parcel in) {
                    return new android.support.v7.widget.Toolbar.SavedState(in, null);
                }
    ...
    public void setContentInsetStartWithNavigation(int insetStartWithNavigation) {
        if (insetStartWithNavigation < 0) {
            insetStartWithNavigation = CitySearchProvider.GET_SEARCH_RESULT_FAIL;
        }
    ...
    public void setContentInsetEndWithActions(int insetEndWithActions) {
        if (insetEndWithActions < 0) {
            insetEndWithActions = CitySearchProvider.GET_SEARCH_RESULT_FAIL;
        }
    ...
    public int getCurrentContentInsetEnd() {
        ...
        return hasActions ? Math.max(getContentInsetEnd(), Math.max(this.mContentInsetEndWithActions, 0)) : getContentInsetEnd();
    }
    ...
    private void measureChildConstrained(View child, int parentWidthSpec, int widthUsed, int parentHeightSpec, int heightUsed, int heightConstraint) {
        ...
        if (childHeightMode != 1073741824 && heightConstraint >= 0) {
            childHeightSpec = MeasureSpec.makeMeasureSpec(childHeightMode != 0 ? Math.min(MeasureSpec.getSize(childHeightSpec), heightConstraint) : heightConstraint, CitySearchProvider.GET_SEARCH_RESULT_SUCC);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width = 0 + Math.max(contentInsetStart, i2);
        collapsingMargins[marginStartIndex] = Math.max(0, contentInsetStart - i2);
        int menuWidth = 0;
        ...
        i = heightMeasureSpec;
        int resolveSizeAndState = View.resolveSizeAndState(Math.max(height, getSuggestedMinimumHeight()), i, childState << 16);
        if (shouldCollapse()) {
            resolveSizeAndState = 0;
        }
        setMeasuredDimension(measuredWidth, resolveSizeAndState);
    }
    ...
    private int getChildHorizontalGravity(int gravity) {
        ...
        switch (hGrav) {
            ...
            case RainSurfaceView.RAIN_LEVEL_THUNDERSHOWER:
                return hGrav;
            default:
                return ld == 1 ? RainSurfaceView.RAIN_LEVEL_THUNDERSHOWER : RainSurfaceView.RAIN_LEVEL_DOWNPOUR;
        }
    }
