...
public class ListViewCompat extends ListView {
    ...
    public int lookForSelectablePosition(int position, boolean lookDown) {
        ...
        if (getAdapter().areAllItemsEnabled()) {
            return (position < 0 || position >= count) ? NO_POSITION : position;
        } else {
            if (lookDown) {
    ...
    public int measureHeightOfChildrenCompat(int widthMeasureSpec, int startPosition, int endPosition, int maxHeight, int disallowPartialChildPosition) {
        ...
        while (i2 < count) {
            ...
            if (childLp.height > 0) {
                heightMeasureSpec = MeasureSpec.makeMeasureSpec(childLp.height, CitySearchProvider.GET_SEARCH_RESULT_SUCC);
            } else {
            ...
            if (returnedHeight >= maxHeight) {
                return (disallowPartialChildPosition < 0 || i2 <= disallowPartialChildPosition || i <= 0 || returnedHeight == maxHeight) ? maxHeight : i;
            } else {
