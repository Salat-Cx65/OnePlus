...
public class LinearSmoothScroller extends SmoothScroller {
    ...
    protected void updateActionForInterimTarget(Action action) {
        ...
        action.update((int) (((float) this.mInterimTargetDx) * 1.2f), (int) (((float) this.mInterimTargetDy) * 1.2f), (int) (((float) calculateTimeForScrolling(TARGET_SEEK_SCROLL_DISTANCE_PX)) * 1.2f), this.mLinearInterpolator);
    }

    private int clampApplyScroll(int tmpDt, int dt) {
        int before = tmpDt;
        tmpDt -= dt;
        return before * tmpDt <= 0 ? SNAP_TO_ANY : tmpDt;
    }
    ...
    public int calculateDtToFit(int viewStart, int viewEnd, int boxStart, int boxEnd, int snapPreference) {
        switch (snapPreference) {
            ...
            case SNAP_TO_ANY:
                ...
                return dtEnd < 0 ? dtEnd : SNAP_TO_ANY;
            case SNAP_TO_END:
