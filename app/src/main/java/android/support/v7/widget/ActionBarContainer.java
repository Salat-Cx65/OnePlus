...
public class ActionBarContainer extends FrameLayout {
    ...
    protected boolean verifyDrawable(Drawable who) {
        if (who != this.mBackground || this.mIsSplit) {
            if (!(who == this.mStackedBackground && this.mIsStacked)) {
                if (!((who == this.mSplitBackground && this.mIsSplit) || super.verifyDrawable(who))) {
                    return false;
                }
            }
        }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (this.mActionBarView == null && MeasureSpec.getMode(heightMeasureSpec) == Integer.MIN_VALUE && this.mHeight >= 0) {
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(Math.min(this.mHeight, MeasureSpec.getSize(heightMeasureSpec)), CitySearchProvider.GET_SEARCH_RESULT_FAIL);
        }
        ...
        if (this.mActionBarView != null) {
            ...
            if (this.mTabContainer != null && this.mTabContainer.getVisibility() != 8 && mode != 1073741824) {
                ...
                setMeasuredDimension(getMeasuredWidth(), Math.min(getMeasuredHeightWithMargins(this.mTabContainer) + topMarginForTabs, mode == Integer.MIN_VALUE ? MeasureSpec.getSize(heightMeasureSpec) : Preference.DEFAULT_ORDER));
            }
        }
    }
