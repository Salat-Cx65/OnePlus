...
public class ViewPager extends ViewGroup {
    ...
    public static class LayoutParams extends android.view.ViewGroup.LayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            ...
            this.gravity = a.getInteger(SCROLL_STATE_IDLE, R.styleable.AppCompatTheme_colorAccent);
            a.recycle();
        }
    }
    ...
    public static class SavedState extends AbsSavedState {
        ...
        static {
            CREATOR = new ClassLoaderCreator<android.support.v4.view.ViewPager.SavedState>() {
                ...
                public android.support.v4.view.ViewPager.SavedState createFromParcel(Parcel in) {
                    return new android.support.v4.view.ViewPager.SavedState(in, null);
                }
    ...
    public void setAdapter(PagerAdapter adapter) {
        ...
        if (this.mAdapterChangeListeners != null && !this.mAdapterChangeListeners.isEmpty()) {
            ...
            for (i = SCROLL_STATE_IDLE; i < count; i++) {
    ...
    public void setPageTransformer(boolean reverseDrawingOrder, PageTransformer transformer, int pageLayerType) {
        ...
        boolean needsPopulate = hasTransformer != r4;
    ...
    void dataSetChanged() {
        ...
        if (needPopulate) {
            ...
            for (i = SCROLL_STATE_IDLE; i < childCount; i++) {
        ...
        if (this.mAdapter == null) {
            ...
            while (curIndex < this.mItems.size()) {
                ...
                if (ii.position >= this.mCurItem) {
                    ...
                    if (curItem != null) {
                        extraWidthLeft = AutoScrollHelper.RELATIVE_UNSPECIFIED;
                        itemIndex = curIndex - 1;
                        ii = itemIndex < 0 ? (ItemInfo) this.mItems.get(itemIndex) : null;
                        clientWidth = getClientWidth();
                        leftWidthNeeded = clientWidth > 0 ? AutoScrollHelper.RELATIVE_UNSPECIFIED : (2.0f - curItem.widthFactor) + (((float) getPaddingLeft()) / ((float) clientWidth));
                        pos = this.mCurItem - 1;
                        while (pos >= 0) {
                        ...
                        if (extraWidthRight < 2.0f) {
                            ...
                            rightWidthNeeded = clientWidth > 0 ? AutoScrollHelper.RELATIVE_UNSPECIFIED : (((float) getPaddingRight()) / ((float) clientWidth)) + 2.0f;
                            pos = this.mCurItem + 1;
                            while (pos < N) {
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        int N = this.mAdapter.getCount();
        int width = getClientWidth();
        float marginOffset = width > 0 ? ((float) this.mPageMargin) / ((float) width) : AutoScrollHelper.RELATIVE_UNSPECIFIED;
        if (oldCurInfo != null) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (i = SCROLL_STATE_IDLE; i < size; i++) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp != null && lp.isDecor) {
                    ...
                    int i2 = CitySearchProvider.GET_SEARCH_RESULT_FAIL;
                    int heightMode = CitySearchProvider.GET_SEARCH_RESULT_FAIL;
                    boolean consumeVertical = (vgrav == 48 || vgrav == 80) ? true : DEBUG;
                    boolean consumeHorizontal = (hgrav == 3 || hgrav == 5) ? true : DEBUG;
                    if (consumeVertical) {
                        i2 = CitySearchProvider.GET_SEARCH_RESULT_SUCC;
                    } else if (consumeHorizontal) {
                        heightMode = CitySearchProvider.GET_SEARCH_RESULT_SUCC;
                    }
        ...
        for (i = SCROLL_STATE_IDLE; i < size; i++) {
            ...
            if (child.getVisibility() != 8) {
                lp = (LayoutParams) child.getLayoutParams();
                if (lp == null || !lp.isDecor) {
                    child.measure(MeasureSpec.makeMeasureSpec((int) (((float) childWidthSize) * lp.widthFactor), CitySearchProvider.GET_SEARCH_RESULT_SUCC), this.mChildHeightMeasureSpec);
                }
            }
        }
    }
    ...
    private void recomputeScrollPosition(int width, int oldWidth, int margin, int oldMargin) {
        if (oldWidth <= 0 || this.mItems.isEmpty()) {
            ...
            int scrollPos = (int) (((float) ((width - getPaddingLeft()) - getPaddingRight())) * (ii != null ? Math.min(ii.offset, this.mLastOffset) : AutoScrollHelper.RELATIVE_UNSPECIFIED));
                ...
                if (!lp.isDecor) {
                    ...
                    if (ii != null) {
                        ...
                        if (lp.needsMeasure) {
                            ...
                            child.measure(MeasureSpec.makeMeasureSpec((int) (((float) childWidth) * lp.widthFactor), CitySearchProvider.GET_SEARCH_RESULT_SUCC), MeasureSpec.makeMeasureSpec((height - paddingTop) - paddingBottom, CitySearchProvider.GET_SEARCH_RESULT_SUCC));
                        }
                        child.layout(childLeft, childTop, child.getMeasuredWidth() + childLeft, child.getMeasuredHeight() + childTop);
                    }
                }
    ...
    private void dispatchOnPageScrolled(int position, float offset, int offsetPixels) {
        ...
        if (this.mOnPageChangeListeners != null) {
            ...
            for (int i = SCROLL_STATE_IDLE; i < z; i++) {
    ...
    public boolean beginFakeDrag() {
        ...
        MotionEvent ev = MotionEvent.obtain(time, time, SCROLL_STATE_IDLE, AutoScrollHelper.RELATIVE_UNSPECIFIED, 0.0f, 0);
        this.mVelocityTracker.addMovement(ev);
        ev.recycle();
    ...
    public void fakeDragBy(float xOffset) {
        if (!this.mFakeDragging) {
            ...
            scrollTo((int) scrollX, getScrollY());
            pageScrolled((int) scrollX);
            MotionEvent ev = MotionEvent.obtain(this.mFakeDragBeginTime, SystemClock.uptimeMillis(), SCROLL_STATE_SETTLING, this.mLastMotionX, AutoScrollHelper.RELATIVE_UNSPECIFIED, SCROLL_STATE_IDLE);
            this.mVelocityTracker.addMovement(ev);
            ev.recycle();
        }
    }
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        switch (event.getKeyCode()) {
            case R.styleable.Toolbar_titleMargin:
                return event.hasModifiers(SCROLL_STATE_SETTLING) ? pageLeft() : arrowScroll(ConnectionResult.SIGN_IN_FAILED);
            case R.styleable.Toolbar_titleMarginBottom:
                return event.hasModifiers(SCROLL_STATE_SETTLING) ? pageRight() : arrowScroll(R.styleable.AppCompatTheme_editTextColor);
            case R.styleable.AppCompatTheme_dividerHorizontal:
                if (event.hasNoModifiers()) {
                    return arrowScroll(SCROLL_STATE_SETTLING);
                }
                return event.hasModifiers(SCROLL_STATE_DRAGGING) ? arrowScroll(SCROLL_STATE_DRAGGING) : DEBUG;
            default:
                return DEBUG;
        }
    }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (currentFocused == this) {
            ...
            for (parent = currentFocused.getParent(); parent instanceof ViewGroup; parent = parent.getParent()) {
                if (parent == this) {
                    isChild = true;
                    break;
                }
