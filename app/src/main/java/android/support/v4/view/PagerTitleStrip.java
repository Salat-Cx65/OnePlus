...
public class PagerTitleStrip extends ViewGroup {
    ...
    public PagerTitleStrip(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(RainSurfaceView.RAIN_LEVEL_SHOWER)) {
            int textColor = a.getColor(RainSurfaceView.RAIN_LEVEL_SHOWER, 0);
            this.mPrevText.setTextColor(textColor);
            this.mCurrText.setTextColor(textColor);
            this.mNextText.setTextColor(textColor);
        }
        this.mGravity = a.getInteger(RainSurfaceView.RAIN_LEVEL_DOWNPOUR, R.styleable.AppCompatTheme_panelMenuListTheme);
        a.recycle();
    ...
    void updateText(int currentItem, PagerAdapter adapter) {
        ...
        int childWidthSpec = MeasureSpec.makeMeasureSpec(Math.max(0, (int) (((float) ((getWidth() - getPaddingLeft()) - getPaddingRight())) * 0.8f)), CitySearchProvider.GET_SEARCH_RESULT_FAIL);
        int childHeightSpec = MeasureSpec.makeMeasureSpec(Math.max(0, (getHeight() - getPaddingTop()) - getPaddingBottom()), CitySearchProvider.GET_SEARCH_RESULT_FAIL);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int childHeightSpec = getChildMeasureSpec(heightMeasureSpec, heightPadding, ListPopupWindow.WRAP_CONTENT);
        int widthSize = MeasureSpec.getSize(widthMeasureSpec);
        int childWidthSpec = getChildMeasureSpec(widthMeasureSpec, (int) (((float) widthSize) * 0.2f), ListPopupWindow.WRAP_CONTENT);
        this.mPrevText.measure(childWidthSpec, childHeightSpec);
        this.mCurrText.measure(childWidthSpec, childHeightSpec);
        this.mNextText.measure(childWidthSpec, childHeightSpec);
