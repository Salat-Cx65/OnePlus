    ...
    private static class SideChannelManager implements Callback, ServiceConnection {
        ...
        private void updateListenerMap() {
            ...
            if (!enabledPackages.equals(this.mCachedEnabledPackages)) {
                ...
                List<ResolveInfo> resolveInfos = this.mContext.getPackageManager().queryIntentServices(new Intent().setAction(ACTION_BIND_SIDE_CHANNEL), MSG_QUEUE_TASK);
                ...
                for (ComponentName componentName2 : enabledComponents) {
                    if (!this.mRecordMap.containsKey(componentName2)) {
                        if (Log.isLoggable(TAG, MSG_RETRY_LISTENER_QUEUE)) {
                            Log.d(TAG, "Adding listener record for " + componentName2);
                        }
        ...
        private boolean ensureServiceBound(ListenerRecord record) {
            ...
            record.bound = this.mContext.bindService(new Intent(ACTION_BIND_SIDE_CHANNEL).setComponent(record.componentName), this, R.styleable.OneplusTheme_op_borderWidth);
            if (record.bound) {
                record.retryCount = 0;
            } else {
        ...
        synchronized (sEnabledNotificationListenersLock) {
            if (enabledNotificationListeners != null) {
                if (!enabledNotificationListeners.equals(sEnabledNotificationListeners)) {
                    ...
                    for (int i = IMPORTANCE_NONE; i < length; i++) {
