...
public class SlidingPaneLayout extends ViewGroup {
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            this.weight = a.getFloat(0, AutoScrollHelper.RELATIVE_UNSPECIFIED);
            a.recycle();
        }
    }
    ...
    private class DragHelperCallback extends Callback {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            return SlidingPaneLayout.this.mIsUnableToDrag ? false : ((LayoutParams) child.getLayoutParams()).slideable;
        }
        ...
        while (i < childCount) {
            ...
            if (child != panel) {
                if (child.getVisibility() != 8) {
                    ...
                    int vis = (clampedChildLeft < left || clampedChildTop < top || Math.min(i2, child.getRight()) > right || Math.min(bottomBound, child.getBottom()) > bottom) ? 0 : RainSurfaceView.RAIN_LEVEL_RAINSTORM;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode != 1073741824) {
            ...
            if (!isInEditMode()) {
                ...
                heightMode = CitySearchProvider.GET_SEARCH_RESULT_FAIL;
                heightSize = 300;
            }
        }
        int layoutHeight = 0;
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (lp.width == -2) {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, CitySearchProvider.GET_SEARCH_RESULT_FAIL);
                } else if (lp.width == -1) {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, CitySearchProvider.GET_SEARCH_RESULT_SUCC);
                } else {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(lp.width, CitySearchProvider.GET_SEARCH_RESULT_SUCC);
                }
                if (lp.height == -2) {
                    ...
                    childHeightSpec = MeasureSpec.makeMeasureSpec(lp.height, CitySearchProvider.GET_SEARCH_RESULT_SUCC);
                }
                ...
                boolean z = widthRemaining < 0 ? 1 : 0;
                lp.slideable = z;
                canSlide |= z;
                if (lp.slideable) {
                ...
                if (child.getVisibility() != 8) {
                    ...
                    if (child.getVisibility() != 8) {
                        ...
                        if (!canSlide || child == this.mSlideableView) {
                            if (lp.weight > 0.0f) {
                                if (lp.width != 0) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(child.getMeasuredHeight(), CitySearchProvider.GET_SEARCH_RESULT_SUCC);
                                } else if (lp.height == -2) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                                } else if (lp.height == -1) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, 1073741824);
                                ...
                                if (canSlide) {
                                    ...
                                    child.measure(MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), CitySearchProvider.GET_SEARCH_RESULT_SUCC), childHeightSpec);
                                }
                            }
                        } else if (lp.width < 0) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        boolean isLayoutRtl = isLayoutRtlSupport();
        if (isLayoutRtl) {
            this.mDragHelper.setEdgeTrackingEnabled(RainSurfaceView.RAIN_LEVEL_SHOWER);
        } else {
        ...
        if (this.mFirstLayout) {
            float f = (this.mCanSlide && this.mPreservedOpenState) ? 1065353216 : null;
            this.mSlideOffset = f;
        }
        for (i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != 8) {
                ...
                if (lp.slideable) {
                    ...
                    lp.dimWhenOffset = ((xStart + lpMargin) + range) + (childWidth / 2) > width - paddingEnd ? 1 : null;
    ...
    private boolean closePane(View pane, int initialVelocity) {
        if (!this.mFirstLayout && !smoothSlideTo(AutoScrollHelper.RELATIVE_UNSPECIFIED, initialVelocity)) {
            return false;
        }
    ...
    private void dimChildView(View v, float mag, int fadeColor) {
        ...
        if (mag > 0.0f && fadeColor != 0) {
            ...
            if (v.getLayerType() != 2) {
                v.setLayerType(RainSurfaceView.RAIN_LEVEL_SHOWER, lp.dimPaint);
            }
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
        int save = canvas.save(RainSurfaceView.RAIN_LEVEL_SHOWER);
