...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
    ...
    public void onNestedScrollAccepted(View child, View target, int nestedScrollAxes) {
        ...
        startNestedScroll(RainSurfaceView.RAIN_LEVEL_SHOWER);
    }
    ...
    protected float getTopFadingEdgeStrength() {
        ...
        return scrollY < length ? ((float) scrollY) / ((float) length) : DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
    }
    ...
    protected float getBottomFadingEdgeStrength() {
        ...
        return span < length ? ((float) span) / ((float) length) : DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mFillViewport && MeasureSpec.getMode(heightMeasureSpec) != 0 && getChildCount() > 0) {
            ...
            if (child.getMeasuredHeight() < height) {
                child.measure(getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight(), ((FrameLayout.LayoutParams) child.getLayoutParams()).width), MeasureSpec.makeMeasureSpec((height - getPaddingTop()) - getPaddingBottom(), CitySearchProvider.GET_SEARCH_RESULT_SUCC));
            }
        }
    }
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    case ConnectionResult.SERVICE_MISSING_PERMISSION:
                        handled = !event.isAltPressed() ? arrowScroll(R.styleable.OneplusTheme_op_borderWidth) : fullScroll(R.styleable.OneplusTheme_op_borderWidth);
                        break;
        ...
        switch (action & 255) {
            case RainSurfaceView.RAIN_LEVEL_DRIZZLE:
                ...
                if (inChild((int) ev.getX(), y)) {
                    ...
                    startNestedScroll(RainSurfaceView.RAIN_LEVEL_SHOWER, 0);
                } else {
                    this.mIsBeingDragged = false;
                    recycleVelocityTracker();
                }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        vtev.offsetLocation(AutoScrollHelper.RELATIVE_UNSPECIFIED, (float) this.mNestedYOffset);
        ViewParent parent;
        switch (actionMasked) {
            case RainSurfaceView.RAIN_LEVEL_DRIZZLE:
                if (getChildCount() == 0) {
            ...
            case RainSurfaceView.RAIN_LEVEL_SHOWER:
                ...
                if (activePointerIndex == -1) {
                    ...
                    if (dispatchNestedPreScroll(0, deltaY, this.mScrollConsumed, this.mScrollOffset, 0)) {
                        ...
                        vtev.offsetLocation(AutoScrollHelper.RELATIVE_UNSPECIFIED, (float) this.mScrollOffset[1]);
        ...
        if ((event.getSource() & 2) != 0) {
            switch (event.getAction()) {
                case DetectedActivity.RUNNING:
                    if (!this.mIsBeingDragged) {
                        float vscroll = event.getAxisValue(ConnectionResult.SERVICE_INVALID);
                        if (vscroll != 0.0f) {
    ...
    private View findFocusableViewInBounds(boolean topFocus, int top, int bottom) {
        List<View> focusables = getFocusables(RainSurfaceView.RAIN_LEVEL_SHOWER);
    ...
    protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        ...
        if (direction == 2) {
            ...
            direction = R.styleable.OneplusTheme_op_borderWidth;
        }
        ...
        return (nextFocus == null || isOffScreen(nextFocus)) ? false : nextFocus.requestFocus(direction, previouslyFocusedRect);
    }
    ...
    public void fling(int velocityY) {
        if (getChildCount() > 0) {
            startNestedScroll(RainSurfaceView.RAIN_LEVEL_SHOWER, 1);
            this.mScroller.fling(getScrollX(), getScrollY(), 0, velocityY, 0, 0, CitySearchProvider.GET_SEARCH_RESULT_FAIL, Preference.DEFAULT_ORDER, 0, 0);
    ...
    private void flingWithNestedDispatch(int velocityY) {
        ...
        if (!dispatchNestedPreFling(AutoScrollHelper.RELATIVE_UNSPECIFIED, (float) velocityY)) {
            dispatchNestedFling(AutoScrollHelper.RELATIVE_UNSPECIFIED, (float) velocityY, canFling);
            fling(velocityY);
        }
    }
