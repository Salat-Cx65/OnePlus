...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        if (max == min) {
            s = AutoScrollHelper.RELATIVE_UNSPECIFIED;
            h = 0.0f;
        } else {
            if (max == rf) {
        ...
        outHsl[0] = constrain(h, (float) AutoScrollHelper.RELATIVE_UNSPECIFIED, 360.0f);
        outHsl[1] = constrain(s, (float) AutoScrollHelper.RELATIVE_UNSPECIFIED, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        outHsl[2] = constrain(l, (float) AutoScrollHelper.RELATIVE_UNSPECIFIED, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    }
