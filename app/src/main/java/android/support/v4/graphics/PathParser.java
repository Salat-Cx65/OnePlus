...
public class PathParser {
    ...
    public static class PathDataNode {
        ...
        PathDataNode(android.support.v4.graphics.PathParser.PathDataNode n) {
            this.mType = n.mType;
            this.mParams = PathParser.copyOfRange(n.mParams, 0, n.mParams.length);
        }

        public static void nodesToPath(android.support.v4.graphics.PathParser.PathDataNode[] node, Path path) {
        ...
        public void interpolatePathDataNode(android.support.v4.graphics.PathParser.PathDataNode nodeFrom, android.support.v4.graphics.PathParser.PathDataNode nodeTo, float fraction) {
        ...
        private static void addCommand(Path path, float[] current, char previousCmd, char cmd, float[] val) {
            int incr = RainSurfaceView.RAIN_LEVEL_SHOWER;
            ...
            switch (cmd) {
                ...
                case R.styleable.AppCompatTheme_windowMinWidthMajor:
                    incr = RainSurfaceView.RAIN_LEVEL_SHOWER;
                    break;
                ...
                case R.styleable.AppCompatTheme_windowFixedWidthMinor:
                    incr = RainSurfaceView.RAIN_LEVEL_RAINSTORM;
                    break;
            ...
            for (int k = 0; k < val.length; k += incr) {
                ...
                switch (cmd) {
                    ...
                    case R.styleable.AppCompatTheme_textAppearancePopupMenuHeader:
                        ...
                        break;
                    case LocationRequest.PRIORITY_LOW_POWER:
                        path.rLineTo(val[k + 0], AutoScrollHelper.RELATIVE_UNSPECIFIED);
                        currentX += val[k + 0];
                        break;
                    case R.styleable.AppCompatTheme_tooltipFrameBackground:
                    ...
                    case R.styleable.AppCompatTheme_windowFixedHeightMinor:
                        ...
                        break;
                    case R.styleable.AppCompatTheme_windowFixedWidthMinor:
                        reflectiveCtrlPointX = AutoScrollHelper.RELATIVE_UNSPECIFIED;
                        reflectiveCtrlPointY = AutoScrollHelper.RELATIVE_UNSPECIFIED;
                        if (previousCmd == 'c' || previousCmd == 's' || previousCmd == 'C' || previousCmd == 'S') {
                    ...
                    case R.styleable.AppCompatTheme_windowMinWidthMajor:
                        ...
                        break;
                    case R.styleable.AppCompatTheme_windowNoTitle:
                        path.rLineTo(AutoScrollHelper.RELATIVE_UNSPECIFIED, val[k + 0]);
                        currentY += val[k + 0];
                        break;
                    default:
                        break;
        ...
        private static void drawArc(Path p, float x0, float y0, float x1, float y1, float a, float b, float theta, boolean isMoreThanHalf, boolean isPositiveArc) {
            ...
            if (isPositiveArc != (sweep >= 0.0d ? 1 : null)) {
                if (sweep > 0.0d) {
        ...
        private static void arcToBezier(Path p, double cx, double cy, double a, double b, double e1x, double e1y, double theta, double start, double sweep) {
            ...
            for (int i = 0; i < numSegments; i++) {
                ...
                p.rLineTo(AutoScrollHelper.RELATIVE_UNSPECIFIED, AutoScrollHelper.RELATIVE_UNSPECIFIED);
