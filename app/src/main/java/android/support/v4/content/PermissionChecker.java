...
public final class PermissionChecker {
    ...
    public static int checkPermission(@NonNull Context context, @NonNull String permission, int pid, int uid, String packageName) {
        ...
        return AppOpsManagerCompat.noteProxyOp(context, op, packageName) != 0 ? PERMISSION_DENIED_APP_OP : 0;
    }
    ...
    public static int checkCallingPermission(@NonNull Context context, @NonNull String permission, String packageName) {
        return Binder.getCallingPid() == Process.myPid() ? PERMISSION_DENIED : checkPermission(context, permission, Binder.getCallingPid(), Binder.getCallingUid(), packageName);
    }
