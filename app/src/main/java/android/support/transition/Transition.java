...
public abstract class Transition implements Cloneable {
    ...
    class AnonymousClass_2 extends AnimatorListenerAdapter {
        ...
        AnonymousClass_2(ArrayMap arrayMap) {
    ...
    public Transition(Context context, AttributeSet attrs) {
        ...
        long duration = (long) TypedArrayUtils.getNamedInt(a, parser, "duration", MATCH_INSTANCE, -1);
        if (duration >= 0) {
            setDuration(duration);
        }
        long startDelay = (long) TypedArrayUtils.getNamedInt(a, parser, "startDelay", MATCH_NAME, -1);
        if (startDelay > 0) {
        ...
        String matchOrder = TypedArrayUtils.getNamedString(a, parser, "matchOrder", MATCH_ID);
        if (matchOrder != null) {
    ...
    protected void createAnimators(ViewGroup sceneRoot, TransitionValuesMaps startValues, TransitionValuesMaps endValues, ArrayList<TransitionValues> startValuesList, ArrayList<TransitionValues> endValuesList) {
        ...
        for (i = 0; i < startValuesListCount; i++) {
            ...
            if (start != null || end != null) {
                boolean isChanged = (start == null || end == null || isTransitionRequired(start, end)) ? true : DBG;
                if (isChanged) {
    ...
    boolean isValidTarget(View target) {
        ...
        if (this.mTargetIds.size() == 0 && this.mTargets.size() == 0) {
            if ((this.mTargetTypes == null || this.mTargetTypes.isEmpty()) && (this.mTargetNames == null || this.mTargetNames.isEmpty())) {
                return true;
            }
        }
    ...
    private void runAnimator(Animator animator, ArrayMap<Animator, AnimationInfo> runningAnimators) {
        if (animator != null) {
            animator.addListener(new AnonymousClass_2(runningAnimators));
            animate(animator);
        }
    }
    ...
    private static <T> ArrayList<T> excludeObject(ArrayList<T> list, T target, boolean exclude) {
        if (target != null) {
            return exclude ? ArrayListManager.add(list, target) : ArrayListManager.remove(list, target);
        } else {
            return list;
        }
    }
    ...
    private ArrayList<Integer> excludeId(ArrayList<Integer> list, int targetId, boolean exclude) {
        if (targetId > 0) {
            return exclude ? ArrayListManager.add(list, Integer.valueOf(targetId)) : ArrayListManager.remove(list, Integer.valueOf(targetId));
        } else {
            return list;
        }
    }
    ...
    private ArrayList<Class> excludeType(ArrayList<Class> list, Class type, boolean exclude) {
        if (type != null) {
            return exclude ? ArrayListManager.add(list, type) : ArrayListManager.remove(list, type);
        } else {
            return list;
        }
    }
    ...
    void captureValues(ViewGroup sceneRoot, boolean start) {
        clearValues(start);
        if (this.mTargetIds.size() > 0 || this.mTargets.size() > 0) {
            if ((this.mTargetNames == null || this.mTargetNames.isEmpty()) && (this.mTargetTypes == null || this.mTargetTypes.isEmpty())) {
                int i;
                View view;
            ...
            if (anim != null) {
                ...
                if (oldInfo != null && oldInfo.mView != null) {
                    if (windowId.equals(oldInfo.mWindowId)) {
                        ...
                        boolean cancel = (!(startValues == null && endValues == null) && oldInfo.mTransition.isTransitionRequired(oldValues, endValues)) ? true : DBG;
                        if (cancel) {
    ...
    private static boolean isValueChanged(TransitionValues oldValues, TransitionValues newValues, String key) {
        ...
        return !oldValue.equals(newValue) ? true : DBG;
    }
        ...
        if (this.mPropagation != null && !transitionValues.values.isEmpty()) {
            ...
            if (propertyNames != null) {
                ...
                for (Object obj : propertyNames) {
                    if (!transitionValues.values.containsKey(obj)) {
                        containsAll = DBG;
                        break;
                    }
                }
    ...
    public String toString() {
        return toString(StringUtils.EMPTY_STRING);
    }
